...
def get_fin_val(df: pd.DataFrame, keys: list, default=0.0) -> float:
    """Looks for multiple potential row names and returns the first found value."""
...
def calculate_breadth_above_50dma(universe_tickers: list = None) -> dict:
    ...
    if universe_tickers is None:
        # Use a representative ~30-stock proxy of the S&P 500 sectors
        universe_tickers = [
            "AAPL", "MSFT", "NVDA", "AMZN", "META", "GOOGL", "TSLA", "BRK-B", "JPM", "V",
            "JNJ", "PG", "XOM", "UNH", "HD", "MA", "AVGO", "LLY", "COST", "MRK",
            "CVX", "BAC", "ABBV", "PFE", "KO", "PEP", "TMO", "MCD", "WMT", "CSCO",
            "ACN", "DHR", "ABT", "ADBE", "CRM", "TXN", "NEE", "PM", "HON", "INTC"
        ]

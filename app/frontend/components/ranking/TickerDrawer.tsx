    ...
    <div className="drawer-overlay" onClick={onClose}>
      <div className="drawer" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div style={{ padding: '8px 16px' }}>
          <div style={{ marginBottom: 4, fontSize: '10px', color: 'var(--col-dim)' }}>SCORE</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span className="score-bar" style={{ color: scoreColor(row.rank_score), fontSize: '14px', letterSpacing: '-1px' }}>
              {'█'.repeat(scoreW)}{'░'.repeat(20 - scoreW)}
            </span>
            <span style={{ color: scoreColor(row.rank_score), fontSize: '18px', fontWeight: 600 }}>{fmt.score(row.rank_score)}</span>
        ...
        <div className="terminal-rule" style={{ margin: '8px 16px' }} />
